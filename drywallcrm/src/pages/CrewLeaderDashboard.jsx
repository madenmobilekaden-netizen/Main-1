...
export default function CrewLeaderDashboard() {
  ...
  if (!profile?.crew_id) {
    return (
      <div style={{ minHeight: "100vh", background: "#0f1114", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16, padding: 24 }}>
        <div style={{ fontSize: 40 }}>🧱</div>
        <div style={{ fontFamily: "'Barlow Condensed'", fontSize: 22, fontWeight: 800, color: "#f5c518", textAlign: "center" }}>NO CREW ASSIGNED</div>
        <div style={{ fontSize: 14, color: "#7a8499", textAlign: "center", maxWidth: 300 }}>You haven't been assigned to a crew yet. Contact your manager.</div>
        <button onClick={signOut} style={{ background: "transparent", border: "1.5px solid #363f50", color: "#7a8499", borderRadius: 8, padding: "10px 24px", fontSize: 15, fontFamily: "'Barlow Condensed'" }}>SIGN OUT</button>
      </div>
    );
  }
        ...
        {jobs.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {jobs.map(job => {
              ...
              const color = STAGE_COLORS[job.stage] || "#7a8499";
              return (
                <div key={job.id} onClick={() => setSelected(job)} style={{
                  ...
                  {job.address && <div style={{ fontSize: 13, color: "#7a8499", marginBottom: 10 }}>{job.address}</div>}
                  {/* Progress */}
                  <div style={{ height: 8, background: "#2a3040", borderRadius: 4, overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${p}%`, background: color, borderRadius: 4, transition: "width .4s" }} />
                  </div>
                  <div style={{ fontSize: 12, color: "#7a8499", marginTop: 4 }}>{p}% complete · {(job.sqft || 0).toLocaleString()} sqft</div>
                </div>
              );
            })}
          </div>
      ...
      {selected && (
        <div onClick={() => { setSelected(null); setConfirmStage(null); }} style={{
          ...
          <div onClick={e => e.stopPropagation()} className="animate-in" style={{
            background: "#1e2329", borderTop: "1.5px solid #363f50", borderRadius: "16px 16px 0 0",
            width: "100%", maxHeight: "92vh", overflowY: "auto", padding: 20,
          }}>
            <div style={{ width: 40, height: 4, background: "#363f50", borderRadius: 2, margin: "0 auto 16px" }} />
            <div style={{ fontFamily: "'Barlow Condensed'", fontSize: 22, fontWeight: 800, marginBottom: 4 }}>{selected.name}</div>
            {selected.address && <div style={{ fontSize: 13, color: "#7a8499", marginBottom: 12 }}>{selected.address}</div>}

            {/* Current stage */}
            <div style={{ marginBottom: 16, padding: "10px 14px", background: "#181c21", borderRadius: 10, border: "1px solid #2a3040" }}>
              <div style={{ fontSize: 11, color: "#7a8499", fontFamily: "'Barlow Condensed'", letterSpacing: ".06em", marginBottom: 4 }}>CURRENT STAGE</div>
              <StageBadge stage={selected.stage} />
            </div>

            {/* Stage buttons — large for job site */}
            <div style={{ fontSize: 12, color: "#7a8499", fontFamily: "'Barlow Condensed'", letterSpacing: ".06em", marginBottom: 8 }}>UPDATE STAGE</div>
              ...
              {STAGES.map(s => {
                ...
                return (
                  <button key={s} onClick={() => !isCurrent && setConfirmStage(s)} style={{
                    ...
                    border: `2px solid ${isCurrent ? color : "#2a3040"}`,
                    color: isCurrent ? color : "#7a8499",
            ...
            {/* Confirm dialog */}
            {confirmStage && (
              <div style={{ background: "#181c21", border: "1.5px solid #f5c51844", borderRadius: 12, padding: 16, marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  <button onClick={() => setConfirmStage(null)} style={{ flex: 1, background: "transparent", border: "1.5px solid #363f50", color: "#7a8499", borderRadius: 10, padding: 14, fontSize: 15, fontFamily: "'Barlow Condensed'" }}>CANCEL</button>
                  <button onClick={() => changeStage(selected.id, confirmStage)} disabled={updating} style={{ flex: 1, background: "#f5c518", border: "none", color: "#111", borderRadius: 10, padding: 14, fontSize: 15, fontWeight: 800, fontFamily: "'Barlow Condensed'", opacity: updating ? 0.7 : 1 }}>
            ...
            {(selected.sheets || []).length > 0 && (
              <div style={{ background: "#181c21", borderRadius: 10, padding: 14, border: "1px solid #2a3040", marginBottom: 14 }}>
                ...
                {(selected.sheets || []).map((s, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: i < selected.sheets.length - 1 ? "1px solid #2a3040" : "none" }}>
                    <span style={{ fontSize: 14, fontWeight: 600 }}>{s.size}</span>
                    <span style={{ fontSize: 14, color: "#f5c518", fontFamily: "'Barlow Condensed'", fontWeight: 700 }}>{s.qty} sheets</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {selected.notes && (
              <div style={{ background: "#181c21", borderRadius: 10, padding: 14, border: "1px solid #2a3040", marginBottom: 14 }}>
                ...
                <div style={{ fontSize: 13, color: "#7a8499", lineHeight: 1.5 }}>{selected.notes}</div>
              </div>
            )}
            ...
            <button onClick={() => { setSelected(null); setConfirmStage(null); }} style={{
              width: "100%", marginTop: 16, background: "transparent", border: "1.5px solid #363f50",
              color: "#7a8499", borderRadius: 10, padding: 14, fontSize: 15, fontFamily: "'Barlow Condensed'",
            }}>CLOSE</button>
          </div>
        </div>
      )}
