  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 28 }}>
        <StatCard label="Jobs This Month" value={thisMonth.length} sub={`${completedThisMonth.length} completed`} accent="#4a90d9" />
        ...
        <StatCard label="Month Crew Pay" value={`$${(monthPay/1000).toFixed(1)}k`} accent="#e07b39" />
        <StatCard label="Month Net Margin" value={`$${(monthMargin/1000).toFixed(1)}k`} accent="#f5c518" />
      </div>

      {/* All-time stats */}
      <div style={{ fontFamily: "'Barlow Condensed'", fontSize: 16, fontWeight: 700, color: "#7a8499", letterSpacing: ".06em", marginBottom: 12 }}>ALL TIME</div>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 28 }}>
        <StatCard label="Total Jobs" value={jobs.length} sub={`${completed.length} completed`} accent="#4a90d9" />
        <StatCard label="Total Revenue" value={`$${(totalRevenue/1000).toFixed(1)}k`} accent="#3db882" />
        <StatCard label="Total Crew Pay" value={`$${(totalPay/1000).toFixed(1)}k`} accent="#e07b39" />
        <StatCard label="Total Net Margin" value={`$${(totalMargin/1000).toFixed(1)}k`} sub={totalRevenue > 0 ? `${Math.round((totalMargin/totalRevenue)*100)}% margin` : ""} accent="#f5c518" />
      </div>

      {/* Jobs table */}
      <div style={{ fontFamily: "'Barlow Condensed'", fontSize: 16, fontWeight: 700, color: "#7a8499", letterSpacing: ".06em", marginBottom: 12 }}>ALL JOBS</div>
      <div style={{ background: "#1e2329", border: "1.5px solid #2a3040", borderRadius: 12, overflow: "hidden" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ background: "#181c21" }}>
                {["JOB","CLIENT","SQ FT","STAGE","CREW","REVENUE","CREW PAY","MARGIN"].map(h => (
                  <th key={h} style={{ padding: "10px 14px", textAlign: "left", fontFamily: "'Barlow Condensed'", fontSize: 11, color: "#7a8499", letterSpacing: ".06em", fontWeight: 700, whiteSpace: "nowrap" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {jobs.map((j, idx) => {
                ...
                return (
                  <tr key={j.id} style={{ borderTop: "1px solid #2a3040", background: idx % 2 === 0 ? "transparent" : "#181c2180" }}>
                    <td style={{ padding: "10px 14px", fontWeight: 600 }}>{j.name}</td>
                    <td style={{ padding: "10px 14px", color: "#7a8499" }}>{j.client || "—"}</td>
                    <td style={{ padding: "10px 14px" }}>{(j.sqft || 0).toLocaleString()}</td>
                    <td style={{ padding: "10px 14px" }}>{j.stage}</td>
                    <td style={{ padding: "10px 14px", color: "#7a8499" }}>{crew?.name || "—"}</td>
                    <td style={{ padding: "10px 14px", color: "#3db882" }}>${rev.toLocaleString(undefined, { maximumFractionDigits: 0 })}</td>
                    <td style={{ padding: "10px 14px", color: "#e07b39" }}>${pay.toLocaleString(undefined, { maximumFractionDigits: 0 })}</td>
